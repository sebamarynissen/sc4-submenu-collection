...
function dive(dir = process.cwd(), obj = Menu, level = 0) {
	let files = fs.readdirSync(dir).filter(file => file !== '_menu.yaml' && file !== 'generate.js');
	let entries = Object.entries(obj);
	for (let i = 0; i < entries.length; i++) {
		let [key, value] = entries[i];
		let sub = files[i];
		if (!sub) console.log(files, i);
		let root = typeof value === 'number' ? value : value[kId];
		if (typeof root === 'number') {
			let target = path.resolve(dir, files[i], '_menu.yaml');
			fs.writeFileSync(target, `name: ${key}\nid: 0x${root.toString(16)}\n`);
		}
		let next = path.join(dir, sub);
		dive(next, value, level+1);
	}
}

dive(path.join(process.cwd(), 'src'));
